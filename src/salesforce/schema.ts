...
export const FIELD_HINTS: Record<string, string> = {
  Opportunity: [
    'location / community / area / project / where the unit is → Building_Community__c (use WHERE, NOT GROUP BY — platform restriction)',
    'deal value / price / amount → Net_Amount__c (NOT Amount); unit selling price → Opportunity_Property__c.cm_Selling_Price__c',
    'new customer price → New_Selling_Price_AED__c (clearly labeled, never add to Nshama selling price)',
    'stage → StageName (values: Generating SalesOffer, Pending Approval, Closed Won, Closed Lost)',
    'won deals → StageName = \'Closed Won\' (or IsWon = true); lost → StageName = \'Closed Lost\'',
    'when closed / sold → CloseDate; booked → Property_Booked_Date__c; order date → Order_Date__c (DEFAULT date for sales summaries)',
    ...
    'direct sale → cm_Agency_Name__c = null AND cm_Agent_Name__c = null',
    'sold by Nshama → Sold_By_Nshama__c = \'NEW SALE\' (use for default sales filters)',
    'which account/customer has most sales/value → query Opportunity, GROUP BY Account.Name, SUM(Net_Amount__c), usually WHERE Sold_By_Nshama__c = \'NEW SALE\'',
    'sales by salesperson → GROUP BY cm_Sales_Person__r.Name; sales by community → WHERE Building_Community__c = \'X\' (not GROUP BY)',
    ...
    'average deal size → AVG(Net_Amount__c); win rate → count won vs total; lost deals → WHERE IsClosed = true AND IsWon = false',
    ...
    'cancellations → Order_Stattus__c IN (\'SMT_CANCELLED\', \'PMT_CANCELLED\', \'BOOKED_CANCELLED\', \'RESERVED_CANCELLED\', \'CANCELLED\')',
    'transfers → Order_Stattus__c = \'TRANSFERED\' (NOT a cancellation)',
    'mandatory exclusions → WHERE (NOT Name LIKE \'%Miscellaneous%\') AND (NOT Name LIKE \'%RTL%\') AND (NOT Name LIKE \'%PK%\') AND (NOT Name LIKE \'%Plot%\') AND (NOT Building_Name__c LIKE \'%Al Qudra%\') AND (NOT Building_Name__c LIKE \'%parking%\') AND Amount != 1 AND CloseDate != 2032-12-28',
  ].join('\n'),
  Account: [
    'location / city → BillingCity; country → BillingCountry',
    'company name → Name; industry → Industry',
    'total revenue from this customer → query Opportunity WHERE Account.Name = X AND Sold_By_Nshama__c = \'NEW SALE\', GROUP BY Account.Name, SUM(Net_Amount__c)',
    'customer type → RecordType.Name (Individual / Corporate / Retail / Agency)',
    'COUNTRY FIELD MAPPING (use the correct field):',
    '  Individual buyer nationality → cm_Nationality__pc (groupable in SOQL)',
    '  Corporate buyer country → cm_Country_Of_Incorporation__c (groupable in SOQL)',
    '  Explicit residence/country of residence → Country_of_Residence_Billing_country__c (NOT groupable — fetch all rows and group in-app)',
    '  Billing country → BillingCountry (standard field, groupable)',
    ...
    'exclude from customer reports → WHERE (NOT Name LIKE \'Test%\') AND (NOT Name LIKE \'Do not update%\') AND (NOT Name LIKE \'%Miscellaneous%\') AND (NOT Name LIKE \'%Contractor%\')',
  ].join('\n'),
