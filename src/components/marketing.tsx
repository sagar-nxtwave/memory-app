    ...
    <div className="min-h-dvh bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-gray-100">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-900">
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between">
          <span className="font-serif text-[18px] font-semibold tracking-tight">Memory</span>
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors px-3 py-2">
              Sign in
            </Link>
            <Link href="/register" className="text-sm font-medium px-4 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl hover:bg-gray-700 dark:hover:bg-gray-100 transition-colors">
              Get started
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-5 pt-20 pb-16 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link href="/register" className="w-full sm:w-auto px-7 py-3.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-semibold rounded-2xl hover:bg-gray-700 dark:hover:bg-gray-100 transition-colors">
            Get started free
          </Link>
          <Link href="/login" className="w-full sm:w-auto px-7 py-3.5 text-sm font-medium text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-800 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
            Sign in →
          </Link>
        </div>
      </section>
      ...
      <section className="py-10 border-y border-gray-100 dark:border-gray-900 overflow-hidden">
        ...
        <div className="flex gap-3 overflow-x-auto px-5 pb-2 scrollbar-hide max-w-5xl mx-auto">
          {[
            ...
            <div key={q} className="shrink-0 px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
              {q}
            </div>
          ))}
        </div>
      ...
      <section className="max-w-5xl mx-auto px-5 py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            { icon: '✦', title: 'Brief Me', desc: 'One tap. Within seconds you understand the project — current status, key numbers, risks, latest decisions.' },
            { icon: '↻', title: 'Catch Me Up', desc: 'Everything that changed since your last visit. New documents, decisions, and items that need your attention.' },
            { icon: '💬', title: 'Chat', desc: 'Ask anything in plain language. Memory searches across all your documents and answers with full context.' },
            { icon: '◷', title: 'Timeline', desc: 'A chronological story of the project — every document, decision, and milestone in one view.' },
          ].map((f) => (
            <div key={f.title} className="p-6 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 hover:border-gray-200 dark:hover:border-gray-700 transition-colors">
      ...
      <section className="border-t border-gray-100 dark:border-gray-900 py-20">
        <div className="max-w-5xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              { step: '01', title: 'Create a space', desc: 'One space per project — Sea Gardens, Antognolla, Holding Company. Separate context for each.' },
              { step: '02', title: 'Upload documents', desc: 'Drop in PDFs, Word docs, Excel files, or add minutes of meeting directly. Memory processes everything automatically.' },
              { step: '03', title: 'Ask anything', desc: 'Memory already knows the context. Brief Me, Catch Me Up, or ask any question in plain language.' },
            ].map((s) => (
        ...
        <div className="max-w-5xl mx-auto px-5 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {['📄 PDF', '📝 Word', '📊 Excel', '📋 CSV', '✏️ Pasted text'].map((t) => (
              <span key={t} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl">
                {t}
              </span>
            ))}
          </div>
      ...
      <section className="border-t border-gray-100 dark:border-gray-900 py-24">
        <div className="max-w-2xl mx-auto px-5 text-center">
          ...
          <Link href="/register" className="inline-block px-8 py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-semibold rounded-2xl hover:bg-gray-700 dark:hover:bg-gray-100 transition-colors">
            Get started free
          </Link>
        </div>
      </section>
      ...
      <footer className="border-t border-gray-100 dark:border-gray-900 py-6">
        <div className="max-w-5xl mx-auto px-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="font-serif text-[16px] font-semibold text-gray-900 dark:text-white tracking-tight">Memory</span>
